...
def extract_word_file_pairs_combined(filename_content):
    ...
    return [(key, count) for key, count in local_count.items()]
...
if __name__ == "__main__":
    ...
    conf = SparkConf().setAppName("Inverted Index Spark with In-Mapper Combiner")
    sc = SparkContext(conf=conf)

    files_rdd = sc.wholeTextFiles(sys.argv[1])

    word_file_pairs = files_rdd.flatMap(extract_word_file_pairs_combined)
